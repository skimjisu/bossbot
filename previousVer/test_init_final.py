...
def init():
	...
	for i in range(fixed_bossNum):
		tmp_fixed_bossData.append(fixed_inputData[i*5:i*5+5]) #카톡
	...
	for j in range(fixed_bossNum):
		...
		fb.append(tmp_fixed_bossData[j][4][13:])               #5 : 카톡On/Off		
...
async def my_background_task():
	...
	while not client.is_closed:		
		...
		if channel != '':
			if endTime == now:
				await dbSave()
				await client.send(client.get_channel(channel), '<보탐봇 화장실 갔다올 시간! 접속완료 후 명령어 입력 해주세요!>', tts=False)
			...
			for i in range(fixed_bossNum):
				if fixed_bossTime[i] <= now :
					...
					await client.send(client.get_channel(channel), embed=embed, tts=False)
			...
			for i in range(bossNum):
				...
				if bossTime[i] <= priv0 and bossTime[i] > priv:
					if basicSetting[3] != '0':
						if bossFlag0[i] == False:
							bossFlag0[i] = True
							await client.send(client.get_channel(channel), bossData[i][0] + ' ' + basicSetting[3] + '분 전 ' + bossData[i][3], tts=False)
				...
				if bossTime[i] <= priv and bossTime[i] > now:
					if basicSetting[1] != '0' :
						if bossFlag[i] == False:
							bossFlag[i] = True
							await client.send(client.get_channel(channel), bossData[i][0] + ' ' + basicSetting[1] + '분 전 ' + bossData[i][3], tts=False)
				...
				if bossMungFlag[i] == True:
					if (bossTime[i]+datetime.timedelta(days=-365)) <= aftr:
						if basicSetting[2] != '0':
							if bossData[i][2] == '0':
								await client.send(client.get_channel(channel), bossData[i][0] + ' 미입력 됐습니다.', tts=False)
								...
								await client.send(client.get_channel(channel), bossData[i][0] + ' 멍 입니다.')
...
async def dbLoad():
	...
	try:
		...
		file.close()
		#await client.send(client.get_channel(channel), '<불러오기 완료>', tts=False)
		LoadChk = 0
		print ("<불러오기 완료>")
	except IOError:
		#await client.send(client.get_channel(channel), '<보스타임 정보가 없습니다.>', tts=False)
		LoadChk = 1
		print ("보스타임 정보가 없습니다.")
...
async def on_ready():
	...
	if basicSetting[6] != "" and basicSetting[7] != "" :
		...
		#print (channel)
		await client.get_channel(basicSetting[7]).send('< 텍스트채널 [' + client.get_channel(int(basicSetting[7])).name + '] 접속완료>', tts=False)
		await client.get_channel(basicSetting[7]).send('< 음성채널 [' + client.get_channel(int(basicSetting[6])).name + '] 접속완료>', tts=False)
...
async def on_message(msg):
	...
	if chflg == 0 :
		...
		await client.send(client.get_channel(channel), '< 텍스트채널 [' + client.get_channel(channel).name + '] 접속완료>', tts=False)
		
		#print (basicSetting[6])
		if basicSetting[6] != "":
			#print ('join channel')
			await JointheVC(client.get_channel(basicSetting[6]), channel)
			await client.send(client.get_channel(channel), '< 음성채널 [' + client.get_channel(basicSetting[6]).name + '] 접속완료>', tts=False)
		else:
			#print ('join no')
			task1 = client.loop.create_task(my_background_task())

		await client.send(client.get_channel(channel), '< 보탐봇 재시작 설정시간 ' + basicSetting[4] + '시 ' + basicSetting[5] + '분입니다. >', tts=False)
		chflg = 1
	...
	if client.get_channel(channel) != msg.channel :
		...
		if message.content.startswith('!채널이동'):
			...
			await client.send(client.get_channel(channel), '< ' + client.get_channel(channel).name + ' 이동완료>', tts=False)
		...
		##################################
			...
			if message.content.startswith(bossData[i][0] +'예상'):
				...
				if len(hello) > len(tmp_msg) + 3 :
					...
					await client.send(client.get_channel(channel), bossData[i][0] + ' 예상 시간을 입력해주세요.')
		...
		##################################
			...
			if message.content.startswith(bossData[i][0] +'삭제'):
				...
				bossMungFlag[i] = (False)
				bossMungCnt[i] = 0
				await client.send(client.get_channel(channel), '<' + bossData[i][0] + ' 삭제완료>', tts=False)
				await dbSave()
		...
		##################################카톡
			...
			if message.content.startswith(bossData[i][0] +'카톡끔'):
				...
				await client.send(client.get_channel(channel), '< ' + bossData[i][0] + ' 카톡 보내기 끔>', tts=False)

			if message.content.startswith(bossData[i][0] +'카톡켬'):
				bossData[i][6] = '1'
				KakaoAlertSave(bossData[i][0], bossData[i][6])
				await client.send(client.get_channel(channel), '< ' + bossData[i][0] + ' 카톡 보내기 켬>', tts=False)
		...
		if message.content.startswith('!카톡끔'):
			...
			await client.send(client.get_channel(channel), '<카톡 보내기 끔>', tts=False)
		...
		if message.content.startswith('!카톡켬'):
			...
			tmp_katalkData.writelines(katalkData)
			tmp_katalkData.close()
			await client.send(client.get_channel(channel), '<카톡 보내기 켬>', tts=False)
		
		##################################
					
		if message.content.startswith('!오빠'):
			#await client.send(channel, '오빠달려려어어어어어어 ', tts=False)
		...
		if message.content.startswith('!분배'):
			...
			if num_sep == 0 :
				await client.send(client.get_channel(channel), '분배 인원이 0입니다. 재입력 해주세요.', tts=False)
			else :
				await client.send(client.get_channel(channel), '1차세금 : ' + str(cal_tax1) + '\n1차 수령액 : ' + str(real_money) + '\n분배자 거래소등록금액 : ' + str(int(real_money/num_sep)) + '\n2차세금 : ' + str(cal_tax2) + '\n인당 실수령액 : ' + str(int(float(int(real_money/num_sep))*0.95)), tts=False)
		...
		if message.content.startswith('!사다리'):
			...
			if num_cong < len(ladder):
				...
				print (result_ladder)
				await client.send(client.get_channel(channel), '----- 당첨! -----\n' + str(result_ladder), tts=False)
			else:
				await client.send(client.get_channel(channel), '추첨인원이 총 인원과 같거나 많습니다. 재입력 해주세요', tts=False)
		...
		if message.content.startswith('!v') or message.content.startswith('!ㅍ'):
			...
			await client.send(client.get_channel(channel), "<@" +id+ ">님이 \"" + sayMessage + "\"", tts=False)
		...
		if message.content.startswith('!명치'):
			await dbSave()
			await client.send(client.get_channel(channel), '<보탐봇 명치 맞고 숨 고르기 중! 접속완료 후 명령어 입력 해주세요!>', tts=False)
			os.system('restart.bat')
			#sys.exit()
		...
		if message.content.startswith('!소환'):
			...
			await client.send(client.get_channel(channel), '< 음성채널 [' + client.get_channel(voice_channel.id).name + '] 접속완료>', tts=False)
		...
		if message.content.startswith('!초기화'):
			...
			await client.send(client.get_channel(channel), '<초기화 완료>', tts=False)
			print ("<초기화 완료>")
		...
		if message.content.startswith('!보스일괄'):
			...
			await client.send(client.get_channel(channel), '<보스 일괄 입력 완료>', tts=False)
			print ("<보스 일괄 입력 완료>")
		...
		if message.content.startswith('!설정확인'):			
			...
			#await client.send(client.get_channel(basicSetting[7]), '< 보탐봇 재시작 설정시간 ' + basicSetting[4] + '시 ' + basicSetting[5] + '분입니다. >', tts=False)
			embed = discord.Embed(
					title = "----- 설정내용 -----",
		...
		if message.content.startswith('!현재시간'):
			await client.send(client.get_channel(channel), datetime.datetime.now().strftime('%H:%M:%S'), tts=False)
print (token)
client.run(token)
#NTAzOTA5NDIyMzg5NTI2NTI5.XRrmZg.T9aKxQ84eAK9mzjb80Yaqd9WLwg